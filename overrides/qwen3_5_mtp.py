...
class Qwen3_5MultiTokenPredictor(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # ---- BEGIN DGX_SPARK_MTP_TOP_K (Prunning/MTP_OPTIMIZATION_PLAN.md) ----
        ...
        # ATTRIBUTE PATH FIXED 2026-08-26. The original probe read `.top_k` off
        # the block or its `experts`. In this vLLM build FusedMoE stores NO
        # `top_k` attribute at all -- `grep -n "self.top_k"` in
        # fused_moe/layer.py returns nothing. The width lives on the config
        # dataclass as `moe_config.experts_per_token` (layer.py:333,
        # config.py:1284), and the router carries its own copy. Reading a
        # non-existent attribute returned None, and the launcher's gate then
        # reported a NO-OP on a run where the override had in fact APPLIED --
        # a false negative that would have killed a valid experiment.
        try:
            _blk = self.layers[0].mlp
            _eff = None
            _exp = getattr(_blk, "experts", None)
            for _obj, _attr in (
                (getattr(_exp, "moe_config", None), "experts_per_token"),
